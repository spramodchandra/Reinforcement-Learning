...
def updateStateActionMatrix(stateActionMatrix,currentState,nextState,action,reward,alpha,gamma):
    action=int(action)
    q=stateActionMatrix[action,currentState[0],currentState[1]]
    ma=-100000
    for i in range(4):
        if stateActionMatrix[i,nextState[0],nextState[1]] >ma:
            ma=stateActionMatrix[i,nextState[0],nextState[1]]
    stateActionMatrix[action,currentState[0],currentState[1]]= q + alpha*(reward+gamma*ma-q)
    return stateActionMatrix

def updatePolicy(policy, utilityMatrix, pos):
    policy[pos] = np.argmax(utilityMatrix[:,pos[0],pos[1]])
    return policy
...
def main():
    ...
    #generate suboptimal policy from optimal policy
    suboptimalPolicy=np.zeros((size,size))
    for i in range(size):
        for j in range(size):
            if np.random.rand()>0.8:
                suboptimalPolicy[i][j]=np.random.randint(low=0, high=4)
            else:
                suboptimalPolicy[i][j]=optimalPolicy[i][j]
    ...
    for j in range(4000):
        ...
        k=0
        while pos!=dest:
            action = suboptimalPolicy[pos]
            move =suboptimalPolicy[pos]
            if swerve[k] > 0.7:
                if swerve[k] > 0.8:
                    if swerve[k] > 0.9:
            ...
            stateActionMatrix = updateStateActionMatrix(stateActionMatrix,pos,newpos,action,rewardMatrix[pos],alpha,gamma)
